...
interface TabTour1Props {
  ...
  onUpdatePouleQualifyCount?: (pouleId: string, qualifyCount: number) => void;
  onUpdatePoulePiste?: (pouleId: string, pisteNumber: number, waveNumber?: number) => void;
  onAutoSimulateTour1: () => void;
  onAdvanceToTour2: () => void;
}

export const TabTour1: React.FC<TabTour1Props> = ({
  poules,
  playersMap,
  settings,
  onUpdateScore,
    ...
    return (
      <div className="bg-white rounded-2xl border border-neutral-200 p-12 text-center space-y-4">
        ...
        <h3 className="text-base font-bold text-neutral-800">
          Les poules du Tour 1 n'ont pas encore été générées
        </h3>
        <p className="text-xs text-neutral-500 max-w-md mx-auto">
          Rendez-vous dans le premier onglet « Inscriptions & Poules » pour inscrire les quilleurs et cliquer sur « Générer les Poules Aléatoirement ».
        </p>
      </div>
    );
  }

  // Calcul du taux de complétion du Tour 1
  ...
  const isTour1Finished = totalThrowsCompleted === totalThrowsNeeded && !hasPendingTieBreaks;
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 sm:p-6 shadow-xs">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-md bg-gray-100 text-gray-800 text-xs font-semibold border border-gray-200">
              <span>Tour 1 : 2 Tirs sur 9 Quilles</span>
              <span>•</span>
              <span>Max 18 Quilles</span>
            </div>
            <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-gray-900">
              Qualifications - Tour 1 ({poules.length} Poules)
            </h2>
            <p className="text-xs text-gray-600 max-w-3xl">
              Saisissez les résultats des 2 tirs par joueur. Les{' '}
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <button
              type="button"
              id="btn-export-judges-tour1-all"
              onClick={() =>
              ...
              title="Générer les feuilles de marque officielles à imprimer pour les juges de piste (1 page A4 par poule)"
            >
            ...
            <button
              ...
              title="Simuler automatiquement les scores de tous les tirs du Tour 1 pour tester"
            >
              <Sparkles className="w-3.5 h-3.5 text-gray-500" />
              <span>Simuler scores Tour 1</span>
            </button>

            <button
              type="button"
              id="btn-advance-to-tour2"
              onClick={onAdvanceToTour2}
            ...
            >
              <span>Passer au Tour 2 ({poules.length * settings.round1QualifiersPerPool} Qualifiés)</span>
              <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>

        {/* Progress Bar & Tour 1/2 next round setup */}
        <div className="mt-4 pt-4 border-t border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs">
          <div className="flex items-center gap-2">
            <span className="text-gray-500">Progression des tirs :</span>
            <span className="font-bold text-gray-900">
              {totalThrowsCompleted} / {totalThrowsNeeded} tirs saisis ({completionPercent}%)
            </span>
          </div>
          ...
          {onUpdateSettings && (
            <div className="flex flex-wrap items-center gap-3 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-200">
              <div className="flex items-center gap-1.5">
                <span className="text-[11px] font-bold text-blue-900">Pistes T1 :</span>
                <input
                  type="number"
                  min={1}
                  max={32}
                  ...
                  title="Nombre de pistes de tir disponibles pour le Tour 1"
                />
              ...
              <span className="text-[11px] font-bold text-gray-700">Suite (T2) :</span>
              <div className="flex items-center gap-1.5">
              ...
              >
                ...
                <div className="bg-gray-50/70 px-4 py-3 border-b border-gray-200 flex flex-wrap items-center justify-between gap-2">
                      ...
                      {onUpdatePoulePiste && (
                        ...
                        >
                          {Array.from({ length: lanesCount }, (_, i) => i + 1).map((num) => (
                            <option key={`opt-piste-${num}`} value={num}>
                              Piste {num}
                            </option>
                          ))}
                        </select>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      type="button"
                      id={`btn-export-judge-poule-${poule.id}`}
                      onClick={() =>
                        exportJudgesScoreSheetPDF({
                          title: 'Olympiades du Rampeau',
                          roundNumber: 1,
                      ...
                      title="Imprimer la feuille de marque pour le juge de piste de cette poule"
                    >
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    <thead className="bg-gray-50/50 text-gray-600 font-bold uppercase text-[10px] tracking-wider border-b border-gray-200">
                      <tr>
                        ...
                        <th className="py-2.5 px-2 text-center w-16">Tir 1</th>
                        <th className="py-2.5 px-2 text-center w-16">Tir 2</th>
                        <th className="py-2.5 px-3 text-center w-18 font-extrabold text-gray-900">
                          Total (/18)
                        </th>
                          ...
                          >
                            ...
                            <td className="py-2 px-2 text-center">
                              <PinScoreInput
                                value={tirs[1]}
                                onChange={(val) =>
                                  onUpdateScore(poule.id, item.scoreObj.playerId, 1, val)
                                }
                                size="sm"
                              />
                            </td>
                            ...
                            <td className="py-2 px-3 text-center">
                              ...
                              >
                                {item.totalScore}
                                <span className="text-[10px] opacity-75 font-normal">/18</span>
                              </span>
                            </td>
